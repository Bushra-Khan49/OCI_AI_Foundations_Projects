...
(Exact numbers reproduce deterministically — see `results/metrics.json` after running `train_classifier.md` or the notebook.)
...
01_Synthetic_Nonlinear_Classification/
  ...
  train_classifier.md         <- CLI script version, no notebook required
  requirements.txt
...
python train_classifier.md                      # default settings
python train_classifier.md --hidden-sizes 1 5 10 20 --noise 0.1   # custom sweep
```
