...
This document summarizes the findings from the gene expression classification pipeline on the Golub et al. (1999) leukemia dataset. The results and metrics analyzed below are generated by executing `train_eval.py` (or equivalently, following the interactive steps in `02_gene_expression_pipeline.ipynb`).

**Dataset Overview:**
The dataset fed into this pipeline consists of 72 leukemia patients (47 with Acute Lymphoblastic Leukemia [ALL], 25 with Acute Myeloid Leukemia [AML]) and 3,571 gene expression measurements per patient.

---

## 1. PCA Projection Interpretation

*(Generated by executing **Step 3: PCA to 2D for visual inspection**)*

![PCA Projection](pca_projection.png)

Because of human limitations, we cannot visually perceive 3,571 dimensions. PCA was used in Step 3 to compress all 3,571 genes down to 2 dimensions purely for visual inspection.
...
*(Generated by executing **Steps 4-6: Setup Pipeline, CV, and Compare Models**)*

![Model Comparison](model_comparison.png)

We evaluated four models using stratified 5-fold cross-validation. To prevent information leakage (giving the training phase a "sneak peek" at the validation cohort's genes), the feature selection step (picking the top 50 genes via ANOVA F-test) was executed *strictly inside* the cross-validation pipeline.
...
*(Generated by executing **Step 8: Save all results to disk**)*
